...
Color rayColorCircle(const Ray& ray, const Sphere& sphere, double t, Color c) {
...
Color rayColorGiven(const Ray& ray, Color c) {
...
void write_image_to_file(int IMG_HEIGHT, int IMG_WIDTH, std::ostream& out, \
    ...
    for (int j = 0; j < IMG_HEIGHT; j++) {
        for (int i = 0; i < IMG_WIDTH; i++) {
            ...
            // Use gradient color based on ray direction
            // Color color = rayGradColor(ray);

            // Check for intersection with the sphere
            double t;
            Color color = rayColorCircle(ray, sphere, t, red);
...
int main(){
    ...
    // render the image
    ofstream out("output_scene.ppm", std::ios::out | std::ios::trunc);
    if (!out) {
