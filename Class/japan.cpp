...
Color rayColor(const Ray& ray) {
    // For simplicity, return a color based on the ray's direction
    float r = (ray.direction().unit_vector().xx() + 1.0f) / 2.0f; // Normalize to [0, 1]
    float g = (ray.direction().unit_vector().yy() + 1.0f) / 2.0f; // Normalize to [0, 1]
    float b = (ray.direction().unit_vector().zz() + 1.0f) / 2.0f; // Normalize to [0, 1]
    return Color(r, g, b);
}
...
Color rayColorFixed(const Ray& ray, Color c) {
    // For simplicity, return the given color
    return c;
}

Color rayColor(const Ray& ray, const Sphere& sphere, double t, Color c, Color bg) {
    if(sphere.intersects(ray, t)) {
        Point intersection = sphere.intersection_point(ray, t);
    ...
    return rayColorFixed(ray, bg); // If no intersection, use gradient color
}
...
void write_image_to_file(int IMG_HEIGHT, int IMG_WIDTH, std::ostream& out, \
    ...
    Color red(1.0, 0.0, 0.0); // Color for the sphere
    Color white(1.0, 1.0, 1.0); // Color for the background
    ...
    for (int j = 0; j < IMG_HEIGHT; j++) {
        for (int i = 0; i < IMG_WIDTH; i++) {
            ...
            double t;
            Color color = rayColor(ray, sphere, t, red, white);
...
int main(){
    ...
    // render the image
    ofstream out("japan.ppm", std::ios::out | std::ios::trunc);
    if (!out) {
        cerr << "Error opening file for writing." << endl;
        return 1;
    }else{
        write_image_to_file(IMG_HEIGHT, IMG_WIDTH, out, camera_origin, focal_length, viewport_u, viewport_v, d_u, d_v, upper_left_corner, pixel_origin);
        out.close();
        cout << "Image written to japan.ppm" << endl;
    }
